...
export const fetchAllStoreReport = async (storeId: string) => {
  ...
  try {
    const response = await fetch(`${apiUrl}/storerevenue/${storeId}/all`, {
      method: "GET",
      headers: {
        Authorization: `${token}`,
    ...
    console.log("chilling", response, storeId);
    if (!response.ok) {
      console.error(
...
export async function fetchDatedStoreCatReport(
  storeId: string,
  catId: string,
  fromDate: string,
  toDate: string
  ...
  try {
    const response = await fetch(
      `${apiUrl}/storerevenue/${storeId}/${catId}/${fromDate}/${toDate}`,
      {
        method: "GET",
        headers: {
          Authorization: `${token}`,
